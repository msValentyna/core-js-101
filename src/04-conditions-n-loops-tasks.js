...
function isCreditCardNumber(ccn) {
  const firstCcnArr = String(ccn).split('');
  const multiplyArr = [];
  const minus9Arr = [];

  if (firstCcnArr.length % 2 === 0) {
    for (let i = 0; i < firstCcnArr.length; i += 1) {
      if (i % 2 === 0) {
        multiplyArr.push(firstCcnArr[i] * 2);
      } else {
        multiplyArr.push(+firstCcnArr[i]);
      }
    }
    for (let i = 0; i < multiplyArr.length; i += 1) {
      if (i % 2 === 0 && multiplyArr[i] > 9) {
        minus9Arr.push(multiplyArr[i] - 9);
      } else {
        minus9Arr.push(multiplyArr[i]);
      }
    }
  } else {
    for (let i = 0; i < firstCcnArr.length; i += 1) {
  ...
  const sumOfnumbers = minus9Arr.reduce((sum, el) => sum + el, 0);
...
// node src/04-conditions-n-loops-tasks.js
